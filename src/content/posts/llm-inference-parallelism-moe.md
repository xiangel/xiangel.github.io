...
> **说明**：本篇讲的是**推理时模型怎么切开**，不是第五篇那种跨实例的请求路由。第一节按「先复制、再按层、再切层内」把六种并行摊开。主线还是：**每暴露一个问题，就引入一种优化，又带出新问题。**

## Table of contents

## 一、先把各种并行摊开

还没切的时候，整网长这样。激活一路是 `[B, S, H]`。后面六种并行，分别下手 B、层号、H、S，或者把 FFN 换成专家。

![未切开时一张卡要装下全部权重、KV 和激活；B / S / H / 层号 / 专家是五条可切的缝](/assets/posts/llm-inference-parallelism-moe/diagram-model-backbone.png)

从最好懂的切法说起。

**DP。** 模型已经装得下，只是请求太多 → 复制几份各接各的。推理里没有梯度，副本之间通常不说话。代价是每份各自缓存 KV，前缀命中会被摊薄。MoE 上会变成「Attention 按请求切、专家按 EP 切」，叫 **DP Attention**，第二节写。

**PP。** 整网太大，最容易想到按层切开 → 前几层一张卡、后几层另一张卡。通信只在阶段边界点对点。新问题是**气泡**：decode 常常一步一个 token，流水线灌不满。第三节写公式。

**TP。** 每一层的矩阵仍然太宽 → 把每个字那条向量沿宽度切开，几张卡同时算同一层的不同段。切的是 H，不是句子有多长。一层里通常先列并行、再行并行，出口一次 all-reduce。第四节写怎么切矩阵。

**SP。** TP 只切了「乘矩阵」。归一化（LayerNorm / RMSNorm）几乎没有权重，却要给每个字做一次，而且每个字自己算、不看邻居。于是可以按字的位置把这段激活分开：GPU 0 做「今 / 天」，GPU 1 做「真 / 好」。**每个词的向量仍然完整**；进注意力前还要拼回整句。SP 寄生在 TP 上，不切 KV。约束写在第五节。

卡在 TP 和 SP 的，通常是 H 和 LayerNorm 没有对上图。上面是「一个词 = 一条向量，TP 切宽度」；下面是「归一化看着整段话，SP 只切这一段激活」。紧接着是分家 → 拼回 → 再分家。
...
**EP。** 前馈变成很多小专家 → 按专家切开，token 按路由飞到专家所在的卡。通信是 dispatch / combine 两次 all-to-all。墙钟看最忙的那张卡。第七、八节写热点和 DeepEP。

同一份结构，切完是六种样子。颜色表示 GPU：哪一块换了颜色，就是这种并行切到的结构。
...
再把镜头推进一层。PP 决定这一层住在哪张卡，DP 决定这张卡接哪几条请求；层内的并行落在不同算子上。
...
收成一张表，方便对照。盯「切开的维」比盯缩写有用。**通信的形状，比「用了几张卡」更能决定这种并行能不能赚钱。** 每一种切开之后立刻要付的账，放在后面各节的「影响与约束」里，这里不重复列。

| 并行   | 切开的维                   | 典型通信                          | 最怕什么                       |
| ------ | -------------------------- | --------------------------------- | ------------------------------ |
| **DP** | batch / 请求               | 推理里通常**没有**                | 前缀缓存被摊薄                 |
| **PP** | layers                     | 阶段边界 **点对点**               | 气泡；batch=1 灌不满           |
| **TP** | 每个词那条向量的宽度 H     | 层内 **all-reduce**               | 跨节点；小消息延迟             |
| **SP** | 哪几个词做归一化           | all-gather / reduce-scatter       | 被叫成「序列并行」的其实是 CP  |
...
第一节把 DP 写成复制接单。这里只补两笔账：缓存被摊薄，以及 MoE 上它为什么会改名叫 **DP Attention**。
...
第一节把 PP 写成按层切开。出了节点再把一层切碎做 TP，通信往往比计算还贵，所以跨节点时按层往下传更常见。这里只补它真正贵的那一笔：**气泡（bubble）**。
...
通信形态变便宜了：不再是每层做一次全集同步，只在阶段边界**点对点**传一次激活。流水线要先灌满才会转起来：第 0 个阶段开始算第 1 个微批次时，后面的阶段只能空手等；最后一个微批次离开第 0 个阶段之后，前面的阶段又要空转到收尾。图里的斜线格，就是这两头空出来的时间。
...
第一节说过 TP 切的是每个词那条向量的宽度 H。一张 H100 是 80 GB，70B 的 bf16 权重就要 140 GB，还没算 KV。下面是 Megatron 怎么切一个线性层。
...
第一节用「今 / 天 / 真 / 好」走过一遍：LayerNorm 每个词自己算，所以可以分家；注意力必须看整句，所以进门前要拼回来。这里只补三件事：必须先有 TP、KV 不降、名字别和 Ulysses 撞。
...
第一节说过：SP 到 Attention 门口就停，真正爆 KV 的时候要切的是注意力看见的那段序列。这就是 **CP**。序列拉到 32k、128k，一张卡也许还装得下 70B 的切片，却装不下这条请求已经生成的 KV。TP 帮不上——GQA / MLA 的 KV 头本来就少，TP 加大以后，每张卡反而要把同一份 KV 再复制一遍。

DeepSpeed 把 Ulysses 也叫 Sequence Parallelism，名字撞了，维没撞。下面是 Ring、Ulysses，以及推理里的 DCP / PCP。
...
Dense 模型的 FFN 是一整块大矩阵，用 TP 切开是划算的：切片仍然够大，all-reduce 摊得下去。MoE 把这块换成**很多个小专家**，每个 token 只激活其中 top-k 个。第一节已经把 EP 写成「按专家切开」；下面用 DeepSeekMoE 把粒度说清楚：

- 整网 **671B** 参数，一次前向大约只激活 **37B**——稀疏比大约 1:18。
- 每层有 **1 个共享专家**（每个 token 必走）和 **256 个路由专家**（每个 token 只选 **top-8**）。

单个 routed expert 的 GEMM 已经很小。如果还用 TP 把它再切成 4 份、8 份，每份更碎，却仍要做 all-reduce。通信占比会高到不划算。所以 token 按路由飞过去，在持有这个专家的卡上把计算做完。
...
怎么选，其实就是第一节那条从简单到复杂的链子，加上约束之后可以写成检查清单：
...
推理并行的主线，还是第一节那条从简单到复杂、"发现问题 → 引入优化 → 带出新问题"的链子：
