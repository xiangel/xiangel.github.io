...
延伸阅读:到这里，调度这条线收束了——模块怎么长出来（第一篇）、KV Cache 怎么管（第二篇）、单机怎么调度（第三篇）、prefill/decode 怎么拆（第四篇）、集群怎么派单（本篇）。下一篇[《大模型推理的并行策略与 MoE》](/posts/llm-inference-parallelism-moe/)往模型内部走：一张卡装不下时，TP / PP / DP / EP 各切哪一维，以及 MoE 把专家并行推到跨节点之后，通信和热点专家怎么接。
