...
def exp_a():
    ...
    ax.plot(offered, offered, "--", color=INK, lw=1.0, alpha=0.4, label="理想（全部服务）")
    ax.plot(offered, colo_g, "-o", color=MUTED, lw=2.0, ms=4, label="colocated（互相干扰）")
    ax.plot(offered, dis_g, "-o", color=ACCENT, lw=2.4, ms=5, label="分离")
    ax.set_xlabel("到达负载（请求 / 秒）")
    ax.set_ylabel("goodput\n（满足 TTFT & TPOT SLO 的 req/s）")
    ax.set_ylim(0, max(dis_g.max(), colo_g.max()) * 1.25)
    ax.grid(True, axis="y", alpha=0.6)
    ax.set_axisbelow(True)
    ax.legend(fontsize=9, loc="upper right", frameon=False)
    _clean(ax)
    ax.set_title(f"实验 A：goodput vs 负载 —— 分离把 SLO 拐点往外推\n"
                 f"（峰值 goodput 高 {dis_g.max()/max(colo_g.max(),1e-9):.1f}×）", fontsize=11)
    fig.tight_layout()
...
def exp_b():
    ...
    ax.text(3, bw_star * 1.12, f"盈亏平衡 ≈ {bw_star:.0f} GB/s", fontsize=8.5, color=INK)
    ax.set_yscale("log")
    ax.set_yticks([1, 5, 12.5, 40, 150, 400])
    ax.set_yticklabels(["1", "5", "12.5", "40", "150", "400"])
    ax.set_ylabel("互连带宽（GB/s，对数）")
    ax.set_xlabel("到达负载（请求 / 秒）")
    ax.set_title("分离的净收益\n（绿 = 分离更优，红 = colocated 更优）", fontsize=10.5)
    cb = fig.colorbar(pcm, ax=ax, label="净收益（ms/s）")
    ...
    for bw, col, a in ramp:
        ...
        ax.plot(loads, loads * transfer_ms, lw=2.0, color=col, alpha=a,
                label=f"KV 传输 @ {bw/1e9:.0f} GB/s")
    ax.plot(loads, loads * avg_P * compute_ms_per_token, "--", color=ACCENT, lw=2.4,
            label="避免的干扰（colocated 代价）")
    ax.set_xlabel("到达负载（请求 / 秒）")
    ax.set_ylabel("成本（引擎时间 ms / 秒）")
    ax.grid(True, alpha=0.5)
    ax.set_axisbelow(True)
    ax.legend(fontsize=8.5, frameon=False)
    _clean(ax)
    ax.set_title("当「避免的干扰」>「KV 传输」时，分离划算", fontsize=10.5)
    fig.suptitle("实验 B：什么时候值得做 PD 分离？（平均 prompt ≈ %d tokens）" % avg_P,
...
def exp_c():
    ...
    ax.plot(prompt, serialized_overhead, color=MUTED, lw=2.2, label="串行（prefill 完再传）")
    ax.plot(prompt, layerwise_overhead, color=ACCENT, lw=2.6, label="逐层（与计算重叠）")
    ax.fill_between(prompt, layerwise_overhead, serialized_overhead, color=ACCENT, alpha=0.10)
    ax.set_xlabel("prompt 长度（tokens）")
    ax.set_ylabel("加到 TTFT 上的 KV 传输开销（ms）")
    ax.grid(True, alpha=0.5)
    ax.set_axisbelow(True)
    ax.legend(fontsize=9, loc="upper left", frameon=False)
    _clean(ax)
    ax.set_title("实验 C：逐层传输把大部分 KV 交接藏进计算\n"
                 "（InfiniBand ≈ 40 GB/s，80 层）", fontsize=11)
    fig.tight_layout()
