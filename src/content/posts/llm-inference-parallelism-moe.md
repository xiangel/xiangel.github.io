...
4. **归一化每个词自己算，不看邻居** → 可以把这段话的归一化分给几张卡，这是 **SP**。每个词的向量仍然完整；注意力仍要看整句。
...
| **SP** | 哪几个词做归一化           | all-gather / reduce-scatter       | 寄生在 TP 上，再削一档激活           | 被叫成"序列并行"的其实是 CP    |
...
读这张表时，盯"切开的维"比盯缩写有用。DP 切的是请求的份数，PP 切的是网络的深，TP 切的是每个词那条向量的宽，SP 切的是**哪几个词做归一化**，CP 切的才是**注意力看见的那段序列**，EP 切的是专家。**通信的形状，比"用了几张卡"更能决定这种并行能不能赚钱。**
...
先把 LayerNorm 说小。给「今」做归一化，只用「今」自己那条向量，不看「天」，也不看「好」。四个词就是四次互不相干的小计算。Dropout 也一样，词和词之间不算账。

所以这段话的归一化，本来就可以分家：GPU 0 做「今 / 天」，GPU 1 做「真 / 好」。**每个词的向量仍然是完整的 H。SP 不把词切窄，只决定「哪几个词在我这张卡上」。**

那为什么还要等 TP 切完才谈这件事？因为只开 TP 的时候，层出口的 all-reduce 会把完整向量再拼回每张卡。于是做 LayerNorm 时，**两张卡都还握着整句四个词**，把同样的归一化再做一遍。权重切开了，这堆暂时放下的向量——也就是激活——没有切开。Prefill 序列一长，这一档就看得见。

**Megatron 的序列并行 SP（Sequence Parallel）** 就是在这里接手：

1. 归一化按词分家，每张卡只留 `S / t` 个词的激活。
2. 进注意力之前 **all-gather** 拼回整句——问「今」的时候必须看见后面三个词。
3. 注意力仍走 TP（按宽度 / 头切开），**不按词摊 KV**。
4. 算完 **reduce-scatter** 再分家，给下一层的归一化。

通信量和原来的 all-reduce 差不多，只是换了一种集体通信，换来的是归一化那段激活每卡只留 `1/t`。DeepSeek-V3 的 attention 写成 **TP4 + SP**，指的就是这套组合。
