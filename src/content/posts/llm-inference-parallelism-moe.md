...
| ------ | -------------------------- | --------------------------------- | ------------------------------------ | ------------------------------ |
| **TP** | hidden / 权重（含词表）    | 层内 **all-reduce**               | 把一份模型装进多卡                   | 跨节点；小消息延迟             |
| **SP** | LayerNorm / Dropout 的序列 | all-gather / reduce-scatter       | 寄生在 TP 上，再削一档激活           | 被叫成"序列并行"的其实是 CP    |
...
TP 只切了线性层的权重。LayerNorm、Dropout 这些算子几乎不吃权重，却把**整段序列的激活**摊在显存里。Prefill 序列一长，这一档就看得见。
...
推理并行的主线，还是那条"发现问题 → 引入优化 → 带出新问题"的链子：
...
- **出节点太贵** → **PP**。改切层，气泡是 `(p−1)/(m+p−1)`；decode 常常 batch=1，几乎串行。
