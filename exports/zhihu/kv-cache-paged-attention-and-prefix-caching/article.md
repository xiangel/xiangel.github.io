...
![Prefill 与 Decode 两阶段示意：Prefill 一次性并行填充 KV Cache，Decode 逐 token 读取不断增长的 KV Cache](https://xiangel.github.io/assets/posts/kv-cache/diagram-prefill-decode.png)
...
![PagedAttention 用 block table 把序列的逻辑块映射到显存池里任意位置的物理块](https://xiangel.github.io/assets/posts/kv-cache/diagram-block-table.png)
...
![Copy-on-Write：整块的共享前缀块从不复制，只有被两个候选共同写入的那个半满尾块被复制一次](https://xiangel.github.io/assets/posts/kv-cache/diagram-cow.png)
...
![Prefix Caching：两个请求共享的前缀块哈希全部命中，只有末尾独特部分需要重算](https://xiangel.github.io/assets/posts/kv-cache/diagram-hash-chain.png)
...
![RadixAttention：把共享前缀组织成基数树，系统提示作为根节点被所有请求共享](https://xiangel.github.io/assets/posts/kv-cache/diagram-radix-tree.png)
...
![ChunkAttention 把共享前缀切成 chunk 存进前缀树，只存一份](https://xiangel.github.io/assets/posts/kv-cache/diagram-chunkattention-memory.png)
...
![ChunkAttention 的 two-phase partition：共享 chunk 批量算一次，私有尾块逐请求算再合并](https://xiangel.github.io/assets/posts/kv-cache/diagram-chunkattention-twophase.png)
...
**原文链接**：https://xiangel.github.io/posts/kv-cache-paged-attention-and-prefix-caching/

**系列上一篇**：[从 Transformer 出发来看推理系统](https://xiangel.github.io/posts/from-causal-lm-to-inference-system/)
