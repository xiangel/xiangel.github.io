...
延伸阅读:到这里，"从一个 causal LM 到一整套分布式推理系统"的主干就讲完了——模块怎么长出来（第一篇）、KV Cache 怎么管（第二篇）、单机怎么调度（第三篇）、prefill/decode 怎么拆（第四篇）、集群怎么派单（本篇）。再往下是更专门的分支：投机解码（speculative decoding）如何和这套调度共存、MoE 的专家并行怎么影响放置、以及多模态请求的调度差异——这些留待后续按需展开。
